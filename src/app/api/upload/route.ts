...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('Using Uploadcare public key:', publicKey.substring(0, 10) + '...');
    ...
    console.log('Uploadcare response headers:', Object.fromEntries(uploadResponse.headers.entries()));

    if (!uploadResponse.ok) {
      const errorText = await uploadResponse.text();
      console.error('Uploadcare error response:', errorText);
      throw new Error(`Upload to Uploadcare failed: ${uploadResponse.status} - ${errorText}`);
    }
